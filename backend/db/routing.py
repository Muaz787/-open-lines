...
async def set_routing_enabled(tenant_id: str, enabled: bool) -> dict:
    res = (await run_query(get_client().table("tenants").update({"routing_enabled": bool(enabled)})
           .eq("id", tenant_id)))
...
async def get_profile(tenant_id: str, phone_number: str | None = None) -> dict | None:
    ...
    res = await run_query(q.limit(1))
    return (res.data or [None])[0]


async def create_profile(tenant_id: str, data: dict) -> dict:
    res = await run_query(get_client().table("call_handling_profiles").insert({**data, "tenant_id": tenant_id}))
    return res.data[0]


async def update_profile(tenant_id: str, profile_id: str, data: dict) -> dict:
    res = (await run_query(get_client().table("call_handling_profiles").update(data)
           .eq("tenant_id", tenant_id).eq("id", profile_id)))
    return (res.data or [{}])[0]
...
async def list_destinations(tenant_id: str) -> list:
    res = (await run_query(get_client().table("routing_destinations").select("*")
           .eq("tenant_id", tenant_id).order("created_at")))
    return res.data or []


async def get_destination(tenant_id: str, destination_id: str) -> dict | None:
    res = (await run_query(get_client().table("routing_destinations").select("*")
           .eq("tenant_id", tenant_id).eq("id", destination_id).limit(1)))
    return (res.data or [None])[0]


async def create_destination(tenant_id: str, data: dict) -> dict:
    res = await run_query(get_client().table("routing_destinations").insert({**data, "tenant_id": tenant_id}))
    return res.data[0]


async def update_destination(tenant_id: str, destination_id: str, data: dict) -> dict:
    res = (await run_query(get_client().table("routing_destinations").update(data)
           .eq("tenant_id", tenant_id).eq("id", destination_id)))
    return (res.data or [{}])[0]
...
async def find_destination_by_hash(tenant_id: str, e164_hash: str) -> dict | None:
    ...
    if not e164_hash:
        return None
    res = (await run_query(get_client().table("routing_destinations").select("*")
           .eq("tenant_id", tenant_id).eq("e164_hash", e164_hash).limit(1)))
    return (res.data or [None])[0]
...
async def list_rules(tenant_id: str, profile_id: str) -> list:
    res = (await run_query(get_client().table("routing_rules").select("*")
...
async def count_rules(tenant_id: str, profile_id: str) -> int:
    res = (await run_query(get_client().table("routing_rules").select("id")
           .eq("tenant_id", tenant_id).eq("profile_id", profile_id)))
    return len(res.data or [])


async def create_rule(tenant_id: str, data: dict) -> dict:
    res = await run_query(get_client().table("routing_rules").insert({**data, "tenant_id": tenant_id}))
    return res.data[0]


async def update_rule(tenant_id: str, rule_id: str, data: dict) -> dict:
    res = (await run_query(get_client().table("routing_rules").update(data)
           .eq("tenant_id", tenant_id).eq("id", rule_id)))
    return (res.data or [{}])[0]


async def delete_rule(tenant_id: str, rule_id: str) -> None:
    (await run_query(get_client().table("routing_rules").delete()
     .eq("tenant_id", tenant_id).eq("id", rule_id)))
...
async def record_transfer_attempt(data: dict) -> dict:
    ...
    if vci:
        existing = (await run_query(client.table("transfer_attempts").select("id")
                    .eq("vapi_call_id", vci).eq("attempt_index", idx).limit(1)))
        if existing.data:
            upd = (await run_query(client.table("transfer_attempts").update(data)
                   .eq("id", existing.data[0]["id"])))
            return (upd.data or [{}])[0]
    ins = await run_query(client.table("transfer_attempts").insert(data))
    return ins.data[0]
...
async def insert_routing_decision(data: dict) -> dict:
    res = await run_query(get_client().table("routing_decisions").insert(data))
    return res.data[0]
...
async def get_latest_transfer_decision(tenant_id: str, vapi_call_id: str) -> dict | None:
    ...
    if not vapi_call_id:
        return None
    res = (await run_query(get_client().table("routing_decisions").select("*")
...
async def get_transfer_attempt(vapi_call_id: str, attempt_index: int = 0) -> dict | None:
    if not vapi_call_id:
        return None
    res = (await run_query(get_client().table("transfer_attempts").select("*")
           .eq("vapi_call_id", vapi_call_id).eq("attempt_index", attempt_index).limit(1)))
    return (res.data or [None])[0]
...
async def set_attempt_duration(attempt_id: str, duration_secs: int) -> None:
    ...
    (await run_query(get_client().table("transfer_attempts").update({"duration_secs": int(duration_secs)})
     .eq("id", attempt_id)))
...
async def create_callback(tenant_id: str, data: dict) -> dict:
    res = await run_query(get_client().table("callback_requests").insert({**data, "tenant_id": tenant_id}))
    return res.data[0]
...
async def list_callbacks(tenant_id: str, status: str = "open") -> list:
    ...
    res = await run_query(q.order("created_at", desc=True))
    return res.data or []


async def update_callback(tenant_id: str, callback_id: str, data: dict) -> dict:
    res = (await run_query(get_client().table("callback_requests").update(data)
           .eq("tenant_id", tenant_id).eq("id", callback_id)))
    return (res.data or [{}])[0]
