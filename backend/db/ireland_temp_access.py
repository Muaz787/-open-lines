...
async def claim(tenant_id: str) -> bool:
    ...
    try:
        rows = ((await run_query(get_client().table(TABLE)
                .insert({"tenant_id": tenant_id}))).data) or []
        return bool(rows)
...
async def clear_action_required(tenant_id: str) -> bool:
    ...
    rows = ((await run_query(get_client().table(TABLE)
            ...
            .eq("tenant_id", tenant_id)
            .not_.is_("action_required_at", "null"))).data) or []
    return bool(rows)
...
async def consume_seconds(*, tenant_id: str, seconds: int) -> dict | None:
    ...
    client = get_client()
    try:
        res = await run_query(client.rpc("ita_consume_seconds",
                         {"p_tenant": tenant_id, "p_seconds": int(seconds)}))
        data = res.data
        ...
        rows = ((await run_query(client.table(TABLE)
                .update({"seconds_used": total, "updated_at": _now()})
                .eq("tenant_id", tenant_id))).data) or []
        return rows[0] if rows else None
...
async def claim_retirement(tenant_id: str) -> bool:
    ...
    rows = ((await run_query(get_client().table(TABLE)
            ...
            .eq("tenant_id", tenant_id)
            .is_("retirement_claimed_at", "null"))).data) or []
    return bool(rows)
...
async def mark_released(tenant_id: str) -> bool:
    rows = ((await run_query(get_client().table(TABLE)
            ...
            .not_.is_("retirement_claimed_at", "null"))).data) or []
    return bool(rows)
