        ...
        <AnimatePresence mode="wait">
          ...
          {stage === 'done' && result && !setup?.needs_regulatory_verification
            ...
            <motion.div key="done" className="success-wrap"
              ...
              {/* Only for someone who skipped it: this panel used to show
                  unconditionally, telling a customer who had just connected a
                  calendar that they still had "one more step". And it connected
                  by linking to the dashboard, which is the hand-off the calendar
                  step itself no longer does. */}
              {!setup?.integration_connected && (
                <div style={{
                  ...
                  <CalendarConnect
                    ...
                    onConnected={() => {
                      trackEvent('calendar_connected',
                        { location: 'onboarding_success', tenant_id: result.tenant_id })
                      void syncSetupState(String(result.tenant_id))
                    }}
                  />
                </div>
              )}
