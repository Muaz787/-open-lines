...
function SubscriptionPage() {
  ...
  const [sidebarLeads, setSidebarLeads]     = useState<{ id: string }[]>([])
  const [sidebarAppts, setSidebarAppts]     = useState<{ id: string }[]>([])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (tRes.ok) setSidebarTenant(await tRes.json())
      if (lRes.ok) setSidebarLeads(await lRes.json())
      if (aRes.ok) { const d = await aRes.json(); setSidebarAppts(Array.isArray(d) ? d : []) }
    }).catch(() => {})
  }, [tenantId, router])
  ...
  const isSubscribed = sidebarTenant?.subscription_status === 'active' || sidebarTenant?.subscription_status === 'canceling'
  const planBadge = () => {
    if (sidebarTenant?.subscription_status === 'active' && sidebarTenant?.subscription_plan)
      return <span className="db-nav-badge db-badge-green">{sidebarTenant.subscription_plan}</span>
    if (!sidebarTenant?.subscription_status || sidebarTenant.subscription_status === 'none' || sidebarTenant.subscription_status === 'incomplete')
      return <span className="db-nav-badge db-badge-amber">Free</span>
    if (sidebarTenant?.subscription_status === 'past_due')
  ...
  const I = (d: string, extra?: string) => () => (
    <svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14">
      <path d={d} {...(extra ? JSON.parse(extra) : {})} />
    </svg>
  )
  const IconDashboard = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><rect x="1.5" y="1.5" width="5" height="5" rx="1.2"/><rect x="8.5" y="1.5" width="5" height="5" rx="1.2"/><rect x="1.5" y="8.5" width="5" height="5" rx="1.2"/><rect x="8.5" y="8.5" width="5" height="5" rx="1.2"/></svg>)
  const IconKB = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><path d="M2 11.5V3.5A1 1 0 013 2.5H9.5L12.5 5.5V11.5A1 1 0 0111.5 12.5H3A1 1 0 012 11.5Z"/><path d="M9 2.5V6H12.5"/></svg>)
  const IconLeads = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.35" width="14" height="14"><path d="M7.5 1.5l1.47 3.29 3.63.52-2.63 2.47.62 3.61L7.5 9.72 4.41 11.4l.62-3.61L2.4 5.31l3.63-.52z" strokeLinejoin="round"/></svg>)
  const IconCalls = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><path d="M2 2.5h4l1.5 3.5-2 1.5a9 9 0 003 3l1.5-2 3.5 1.5V13a1 1 0 01-1 1C5.5 14 1 9.5 1 3.5a1 1 0 011-1z" strokeLinejoin="round"/></svg>)
  const IconCalendar = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><rect x="1.5" y="2.5" width="12" height="11" rx="1.5"/><path d="M1.5 6.5h12M5 1.5v2M10 1.5v2"/></svg>)
  const IconSettings = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><circle cx="7.5" cy="4.8" r="2.3"/><path d="M2 13.5c0-3.04 2.46-5.5 5.5-5.5s5.5 2.46 5.5 5.5" strokeLinecap="round"/></svg>)
  const IconBilling = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><path d="M1.5 7.5L7.5 2l6 5.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M3 7v5.5a.5.5 0 00.5.5H6V9.5h3v3.5h2.5a.5.5 0 00.5-.5V7" strokeLinejoin="round"/></svg>)
  const IconUsage = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><rect x="1.5" y="9.5" width="2.5" height="4" rx="0.5"/><rect x="6.25" y="6" width="2.5" height="7.5" rx="0.5"/><rect x="11" y="2.5" width="2.5" height="11" rx="0.5"/></svg>)
  const IconSignOut = () => (<svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.4" width="14" height="14"><path d="M6 2H3a1 1 0 00-1 1v9a1 1 0 001 1h3"/><path strokeLinecap="round" d="M10 10l3-3-3-3M13 7H6"/></svg>)
    ...
    <>
      ...
      {sidebarTenant && (
        <div className="db-clinic-sw">
          <div className="db-clinic-name">{sidebarTenant.business_name}</div>
          <div className="db-clinic-tag">{sidebarTenant.industry} · Active</div>
        </div>
      )}
      <div className="db-sidebar-nav">
        <div className="db-nav-label">Main</div>
        <Link href={`/dashboard/${tenantId}`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}><IconDashboard />Dashboard</Link>
        <Link href={`/dashboard/${tenantId}/knowledge-base`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}><IconKB />Knowledge Base</Link>
        <Link href={`/dashboard/${tenantId}/leads`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}>
          <IconLeads />Leads
          {sidebarLeads.length > 0 && <span className="db-nav-badge db-badge-red">{sidebarLeads.length}</span>}
        </Link>
        <Link href={`/dashboard/${tenantId}/calls`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}><IconCalls />Calls</Link>
        {sidebarTenant && BOOKING_INDUSTRIES.has(sidebarTenant.industry) && (
          <Link href={`/dashboard/${tenantId}/calendar`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}>
            <IconCalendar />Calendar
            {sidebarAppts.length > 0 && <span className="db-nav-badge db-badge-green">{sidebarAppts.length}</span>}
          </Link>
        )}
        <div className="db-nav-label">Account</div>
        <Link href={`/dashboard/${tenantId}/settings`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}><IconSettings />Settings</Link>
        <Link href={`/dashboard/${tenantId}/subscription`} className="db-nav-item active" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}>
          <div className="db-nav-indicator" /><IconBilling />Billing &amp; Payments{planBadge()}
        </Link>
        <Link href={`/dashboard/${tenantId}/usage`} className="db-nav-item" onClick={mobile ? () => setMobileMenuOpen(false) : undefined}><IconUsage />Usage</Link>
        {mobile && (
          <div style={{ marginTop: 8, borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 8 }}>
            <button className="db-nav-item" onClick={handleLogout} style={{ color: '#f87171' }}><IconSignOut />Sign out</button>
          </div>
        )}
      </div>
      {!isSubscribed && (
      ...
      {!mobile && (
        <div className="db-sidebar-footer">
          <button className="db-user-row" onClick={handleLogout} title="Sign out">
            <div className="db-user-av">{userName ? userName.split(' ').slice(0,2).map(w=>w[0]).join('').toUpperCase() : (userEmail||'?')[0].toUpperCase()}</div>
            <div className="db-user-info">
              <div className="db-user-name">{userName || sidebarTenant?.business_name || 'Account'}</div>
      ...
      <div className="db-main">
        ...
        <div className="db-mobile-topbar">
          <div className="db-mobile-logo">
            <div className="db-logo-icon"><LogoMark size={22} /></div>
            <span className="db-mobile-logo-name">Billing</span>
          </div>
        ...
        {/* Legacy hamburger dropdown (kept to avoid breaking JSX structure — hidden) */}
        <AnimatePresence>
          {false && (
            <motion.div
            ...
            >
            <Link href={`/dashboard/${tenantId}`} className="db-mobile-item" onClick={() => setMobileMenuOpen(false)}>
            ...
            <Link href={`/dashboard/${tenantId}/settings`} className="db-mobile-item" onClick={() => setMobileMenuOpen(false)}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
