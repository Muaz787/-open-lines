...
- [x] **New industries shipped (2026-07-06)** — **Automotive** (one intent-branching template: sales / service & repair / body & collision), **Insurance** (compliance-first: intake + route only, no advice/quotes/binding, claims escalated), and **Government & Public Office** (`public_office` — non-partisan elected-official's office covering federal MP / provincial MPP-MLA / municipal councillor / school board trustee; casework + service requests, jurisdiction routing). Each has a template + qualification fields; all three industry whitelists mirrored; website classifier detects them (and no longer auto-detects `parliament`, which is kept as a legacy alias for existing tenants). No schema/migration; existing tenants unaffected.
