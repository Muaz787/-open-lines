-- ============================================================================
-- Open Lines — CONSOLIDATED DATABASE SCHEMA (single bring-up file)
-- ============================================================================
-- Run this ONE file in the Supabase SQL editor (or via psql) to bring a
-- database fully up to date. It is the union of:
--     backend/db/schema.sql   (core tables + later ALTERs)
--   + migrations/001..007      (payments/short-links, Square, refunds, country, Zapier)
--   + payments base table      (reconstructed — see note below)
--   + Microsoft calendar cols  (were only ever documented, never migrated)
--
-- Every statement is idempotent (create ... if not exists / add column if not
-- exists / drop not null), so this file is SAFE TO RE-RUN and safe on an
-- existing production database — already-present objects are left untouched.
--
-- Ordering matters: core tables (Section A) are created before the payments
-- table and the ALTERs that depend on them.
--
-- NOTE on `payments`: its CREATE TABLE was never tracked in the repo (the prod
-- table was created early by hand). Section B reconstructs it from the app's
-- insert shape (backend/routers/tools.py) in its PRE-migration form; the
-- Section C migrations then evolve it (provider column, nullable
-- stripe_account_id, refunded_at). On production the CREATE is a no-op because
-- the table already exists, so the reconstruction only matters for a fresh DB.
-- ============================================================================


-- ============================================================================
-- SECTION A — Core schema (from backend/db/schema.sql)
-- ============================================================================
...
-- ============================================================================
-- SECTION B — payments base table (reconstructed; pre-migration form)
-- Section C migrations below add `provider`, make stripe_account_id nullable,
-- and add `refunded_at`.
-- ============================================================================
...
-- ============================================================================
-- SECTION C — Numbered migrations 001–007 (verbatim, in order)
-- ============================================================================


-- ----------------------------------------------------------------------------
-- migrations/001_payment_short_links.sql
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- migrations/002_square_integration.sql
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- migrations/003_payments_stripe_account_nullable.sql
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- migrations/004_payments_refunded_at.sql
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- migrations/005_cancellation_refund_policy.sql
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- migrations/006_tenant_country.sql
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- migrations/007_zapier_integration.sql
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- SECTION D — Microsoft (Outlook) Calendar columns
-- Used by backend/routers/calendar.py + payments.py but never migrated until now.
-- ============================================================================
...
-- ============================================================================
-- SECTION G — Prompt system v2 (owner operating layer + custom rebuild)
-- Structured business profile that renders into the owner-operating layer
-- (defaults applied at build when null), plus persisted business_description
-- and a reusable custom prompt base with a [[KNOWLEDGE_CONTEXT]] slot so custom
-- tenants can refresh KB on rebuild instead of being frozen at onboarding.
-- ============================================================================
